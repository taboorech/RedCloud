...
function CircleButton({ className = "", to, isLink, isNavLink, title, children, onClick }) {
  ...
  if(isLink) {
    ...
    content = <button className={classNamesHandler("Circle-button btn-floating", className)} onClick={onClick} title={title}>{children}</button>
  }
